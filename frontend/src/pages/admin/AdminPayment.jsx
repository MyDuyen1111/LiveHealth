...
const AdminPayment = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (modal === 'create') {
        await adminPaymentApi.create(form);
        addToast(t('admin.addSuccess'));
      } else {
        await adminPaymentApi.update(modal.id, form);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await adminPaymentApi.delete(id);
  ...
  return (
    <div>
      <div className="adm-page-header">
        <h1 className="adm-page-title">{t('admin.payment')}</h1>
        ...
        <div className="adm-modal-overlay" onClick={close}>
          <div className="adm-modal" onClick={e => e.stopPropagation()}>
            <h3 className="adm-modal-title">
              {modal === 'create' ? `${t('admin.add')} ${t('admin.payment')}` : `${t('admin.edit')} ${t('admin.payment')}`}
            </h3>
